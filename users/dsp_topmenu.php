			...
			<ul class="nav navbar-nav nav-apps">
				...
				while ($menu = mysql_fetch_array($qry_apps)) {
					...
					if($menu['show_in_topmenu'] == 1){
						...
						{
							?>
								<li class="<?= $menu['is_current'] == 1 ? 'active' : '' ?>"><a href="<?= $relative_url ?>"><?= ($menu['fa_icon'] == '' ? '' : '<i class="fa fa-'.$menu['fa_icon'].'" '. ($menu['theme_color'] == '' ? '' : 'style="color:#'.$menu['theme_color'].';"') . '></i> ') ?><?= htmlentities($description) ?></a></li>
							<?php
						}
						else
						{
							?>
								<li class="subs <?= $menu['is_current'] == 1 ? 'active' : '' ?>"><a href="#" id="btnmenu<?= $menu['id_app'] ?>" data-toggle="collapse" data-target="#submenu<?= $menu['id_app'] ?>" aria-expanded="<?= ($menu['is_current'] == 1 ? 'true' : 'false') ?>" class="<?= ($menu['is_current'] == 1 ? '' : 'collapsed') ?>"><?= ($menu['fa_icon'] == '' ? '' : '<i class="fa fa-'.$menu['fa_icon'].'" '. ($menu['theme_color'] == '' ? '' : 'style="color:#'.$menu['theme_color'].';"') . '></i> ') ?><?= htmlentities($description) ?></a>
