						...
						switch($grabfile['type']){
							case 'youtube-dl':
								if(strpos($grabfile['full_path'], '.mp3') > 0){
									$grabbedfile = shell_exec('/usr/local/bin/youtube-dl --no-part --extract-audio --audio-format mp3 --audio-quality 0 -o "' . str_replace('.mp3', '.%(ext)s', $grabfile['full_path']) . '" ' . $grabfile['full_url']);
								}
								else {
