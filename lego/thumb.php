...
$thumbnail = $settings->val('manuals_directory_thumbs', '') . $thumbWidth . ($square == 1 ? 'square' : 'prop') . '/' . $src;

if(
	file_exists($settings->val('manuals_directory', '') . $src)
	&&
	(
		!file_exists($thumbnail)
		||
		(filemtime($thumbnail) < filemtime($settings->val('manuals_directory', '') . $src))
	)
)
{
	...
	if(stripos($src, '.jpg') > 0 || stripos($src, '.jpeg') > 0 || stripos($src, '.png') > 0)
	{
		...
		{
			$img = imagecreatefromjpeg($settings->val('manuals_directory', '') . $src);
		}
		else if(stripos($src, '.png') > 0)
		{
			$img = imagecreatefrompng($settings->val('manuals_directory', '') . $src);
		}
